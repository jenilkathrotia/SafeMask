...
We do not have hand-drawn ground truth for ACDC images. So we use a
strong Canny as our reference, the way cv08 does in its demos. For
each detector we record:
...
as edges, which is too many. You can see the noise in the output,
mostly in fog and snow images. Sobel at P90 drops to 11% but cuts long
edges into pieces. Both Cannys land at 3 to 5%, which is what cv08
says a clean edge map should look like.
...
finds. The IoU for both Sobels is also low, but for a different
reason: their edges are thick (multiple pixels wide) so they cover
much more area than the thin Canny edges.

`runtime_by_detector.png`. cv2.Canny is about 120 times faster than
our Python port (2 ms vs 255 ms per image). For running on all 4006
ACDC images, we should use cv2.

## What we picked

We are using Canny for SafeMask. We use the Project 3 port to check
our work matches the lecture, and cv2.Canny for the full 4006 image
batch because it is so much faster. We are not using thresholded Sobel
as our final edge layer. Sobel still helps as a rough gradient map,
but its edges are too thick and too noisy to be the final answer.
